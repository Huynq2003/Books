...
const bookSlice = createSlice({
  ...
  reducers: {
    ...
    deleteBook: (state, action: PayloadAction<string>) => {
      Object.keys(state.booksByGrade).forEach(grade => {
        state.booksByGrade[grade] = state.booksByGrade[grade].filter(book => book.id !== action.payload);
      });
    },
    updateBook: (state, action: PayloadAction<Book>) => {
      Object.keys(state.booksByGrade).forEach(grade => {
        state.booksByGrade[grade] = state.booksByGrade[grade].map(book =>
