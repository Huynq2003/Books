...
interface BookState {
  ...
  filteredBooks: Record<string, Book[]>; // Trạng thái để lưu trữ sách đã lọc
}
...
const loadFromLocalStorage = (): BookState => {
  try {
    ...
    return { booksByGrade: {}, bookCategories: [] }; // Trả về state mặc định nếu có lỗi
  }
};
...
const bookSlice = createSlice({
  ...
  reducers: {
    ...
    // Thêm reducer để lọc sách theo thể loại
