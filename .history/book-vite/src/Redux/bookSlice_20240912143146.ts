...
const bookSlice = createSlice({
  ...
  initialState,
  reducers: {
    setBookCategories: (state, action: PayloadAction<BookCategory[]>) => {
      state.bookCategories = action.payload;
    },
    setBooksByGrade: (state, action: PayloadAction<Record<string, Book[]>>) => {
      state.booksByGrade = action.payload;
    },
    filterBooksByCategory: (state, action: PayloadAction<{ grade: string; category: string }>) => {
      ...
      if (state.booksByGrade[grade]) {
        state.filteredBooks[grade] = state.booksByGrade[grade].filter(book => book.category === category);
      } else {
        state.filteredBooks[grade] = [];
      }
    },
  },
});

export const { setBookCategories, setBooksByGrade, filterBooksByCategory } = bookSlice.actions;
