...
const bookSlice = createSlice({
  ...
  reducers: {
    ...
    filterBooksByCategory(state, action: PayloadAction<{ grade: string; category: string }>) {
      ...
      console.log("Lọc sách cho lớp:", grade);  // Kiểm tra lớp học
      console.log("Thể loại:", category);  // Kiểm tra thể loại
    
      // Kiểm tra sách có thuộc lớp này không
      const booksForGrade = state.booksByGrade[grade] || [];
      console.log("Sách của lớp:", booksForGrade);
    
      state.filteredBooks[`${grade}-${category}`] = booksForGrade.filter(book => {
        console.log("Sách:", book.title, "Thể loại sách:", book.category);  // Kiểm tra thể loại sách
        return book.category === category;
      });
    
      console.log("Sách đã lọc:", state.filteredBooks[`${grade}-${category}`]);
    }
