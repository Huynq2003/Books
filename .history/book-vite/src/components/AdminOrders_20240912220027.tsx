...
const AdminOrders: React.FC = () => {
  // Lấy thông tin đơn hàng từ Redux
  const orders = useSelector((state: RootState) => state.orders.allOrders); // Điều chỉnh theo cấu trúc state của bạn

  const [formattedOrders, setFormattedOrders] = useState<OrderInfo[]>([]);

  useEffect(() => {
    // Nếu lấy từ localStorage
    if (orders) {
