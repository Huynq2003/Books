...
import { Button, Rate, Modal, notification, Card, List } from 'antd';
...
const BookDetail: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (foundBook) {
      // Filter books that are in the same category but exclude the current book
      const related = allBooks.filter(b => b.category === foundBook.category && b.id !== foundBook.id);
            ...
            <List
              ...
              renderItem={relatedBook => (
                <List.Item>
                  <Card
                    hoverable
                    cover={<img alt={relatedBook.title} src={relatedBook.image} />}
                    actions={[
                      <Button type="primary" onClick={() => dispatch(addToCart(relatedBook))}>Thêm vào giỏ</Button>,
                    ]}
                  >
