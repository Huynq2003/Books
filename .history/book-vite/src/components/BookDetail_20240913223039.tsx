        ...
        >
          {relatedBooks.slice(0, 8).map((relatedBook) => (
            <Card
              key={relatedBook.id}
              hoverable
              cover={<img alt={relatedBook.title} src={relatedBook.image} className="object-cover custom-card-image w-[250px] h-[250px]" />}
              className="mx-2"
            >
              <Card.Meta title={relatedBook.title} description={`${relatedBook.price} VND`} />
              <div className="mt-4">
                <Link to={`/books/${book.id}/${book.id}`} className="text-blue-500 hover:underline">
                  Xem chi tiết
                </Link>
              </div>
            </Card>
