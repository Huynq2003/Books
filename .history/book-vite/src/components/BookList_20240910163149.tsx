...
const BookList: React.FC<BookListProps> = ({ books }) => {
  ...
  // Hiển thị tất cả sách khi component được render lần đầu
  useEffect(() => {
  ...
  const filterBooks = (grade?: string, term: string = '') => {
    ...
    // Sắp xếp sách theo tên (có thể thay đổi theo yêu cầu, ví dụ như theo giá)
    updatedBooks.sort((a, b) => a.title.localeCompare(b.title)); // Sắp xếp theo tên sách
            ...
            <Card
              ...
              cover={
                <img 
                  alt={book.title} 
                  src={book.image} 
                  style={{ height: '200px', objectFit: 'cover' }} // Thêm style cho ảnh để căn chỉnh tốt hơn
                />
              }
