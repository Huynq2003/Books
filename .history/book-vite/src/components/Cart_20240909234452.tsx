              ...
              <List.Item >
                ...
                >
                  <div className='flex gap-[35px] items-center '>
                    <p className='pr-36'>Tác giả: {item.author}</p>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingRight:25 }}>
                      <Button onClick={() => dispatch(decreaseQuantity(item.id))}>-</Button>
                      <span style={{ margin: '0 10px' }}>{item.quantity}</span>
                      <Button onClick={() => dispatch(increaseQuantity(item.id))}>+</Button>
                    </div>
                    <p className='pr-36'>Giá: ${item.price}</p>
                    <Button className='pr-36' style={{border:1, borderStyle:'solid', borderColor:'red'}} onClick={() => dispatch(removeFromCart(item.id))}>
                      Xóa
                    </Button>
                    <Checkbox 
