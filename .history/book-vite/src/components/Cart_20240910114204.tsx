        ...
        <>
          <List
            ...
            renderItem={(item: Book) => ( // Xác định kiểu của item là CartItem
              <List.Item >
                <div style={{ lineHeight: 3, display: 'flex', justifyContent:'space-evenly'}}
                  title={`${item.title} (x${item.quantity})`}
                >
                  <div className='flex gap-[35px] items-center items-center '>
                    <p>{item.title}</p>
                    <p className=''>Tác giả: {item.author}</p>
                    ...
                    <Button className='' style={{border:1, borderStyle:'solid', borderColor:'red'}} onClick={() => dispatch(removeFromCart(item.id))}>
                      Xóa
                    </Button>
                    <Checkbox 
