    ...
    <header className="bg-white shadow-md border-2 p-4">
      <div className="container mx-auto flex items-center justify-between">
        ...
        <Link to="/" className='flex flex-col items-center'>
          ...
          <span className="ml-2 text-2xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-purple-400 via-pink-500 to-red-500">
            {['H', 'U', 'Y', ' ', 'B', 'O', 'O', 'K', 'S'].map((letter, index) => (
              <span
                key={index}
                style={{ display: 'inline-block', transform: `rotate(${index % 50 === 0 ? 10 : 10}deg)` }}
              >
                {letter}
              </span>
            ))}
