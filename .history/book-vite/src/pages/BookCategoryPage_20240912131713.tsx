...
const FilteredBooksPage: React.FC = () => {
  ...
  // Kiểm tra grade và category trước khi sử dụng
  const filteredBooks = grade && category 
    ? booksByGrade[grade]?.filter(book => book.category === category) || []
    : [];

  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-6">Sách cho {category} lớp {grade}</h1>
      <div>
