...
const Checkout: React.FC = () => {
  ...
  const [selectedItems, setSelectedItems] = useState<any[]>([]); // Quản lý sản phẩm đã chọn

  // Tính tổng số tiền các sản phẩm đã chọn
  ...
  const handleFinish = (values: any) => {
    if (selectedItems.length === 0) {
      message.error('Chưa có sản phẩm nào được chọn để thanh toán!');
      return;
    }
      ...
      {cartItems.length === 0 ? (
        ...
        <>
          ...
          <div className="mt-4 flex justify-between items-center">
            <h3 className="text-lg font-semibold">Tổng cộng: {formattedTotalAmount}.000 VND</h3>
            <Button
              type="primary"
