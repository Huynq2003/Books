...
const Checkout: React.FC = () => {
  ...
  const handleFinish = (values: any) => {
    ...
    setTimeout(() => {
      ...
      console.log('Các sản phẩm trong giỏ:', selectedItems);
  ...
  const handleConfirmOrder = () => {
    ...
    // Cập nhật danh sách giỏ hàng để làm mới màn hình
    dispatch(removeFromCart(selectedItems.map((item: any) => item.id)));
  };

  const handleCancelOrder = () => {
    // Hủy đơn hàng, có thể thực hiện thêm các hành động khác nếu cần
    setIsConfirmationModalVisible(false);
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
            ...
            <Radio.Group
              onChange={(e) => {
                ...
                if (e.target.value !== 'bankTransfer') {
                  setSelectedBank(undefined); // Reset bank selection if not bank transfer
                }
              }}
            >
      ...
      >
        ...
        <p className='flex gap-1'><strong>Phương thức thanh toán:</strong> <p className='capitalize'>{paymentMethod === 'bankTransfer' ? `Chuyển khoản ngân hàng (${selectedBank})` : 'Thanh toán khi nhận hàng'}</p></p>
        <div className="mt-4">
