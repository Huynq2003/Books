...
const OrderConfirmation: React.FC = () => {
  ...
  const saveOrderInfo = (newOrderInfo:any) => {
    ...
    existingOrders.push(newOrderInfo);
  ...
  useEffect(() => {
    ...
    if (savedOrders.length > 0) {
      // Lấy đơn hàng mới nhất (hoặc bạn có thể chọn đơn hàng cụ thể nếu cần)
      setOrderInfo(savedOrders[savedOrders.length - 1]);
    }
  }, []);
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto " style={{lineHeight:2}}>
      ...
      <div className="mt-4">
        ...
        <p className='text-xl'><strong>Tổng cộng thanh toán:</strong> {orderInfo.formattedTotalAmount}.000 VND</p>        </div>
      </div>
      <div className="mt-4">
